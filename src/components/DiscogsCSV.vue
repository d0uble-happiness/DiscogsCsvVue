...
// import { chooseFiles } from './FileUpload.vue'
...
async function parseCsvToArray(file: File): Promise<string[]> {
...
if (fileInputElement) {
  fileInputElement.addEventListener("change", async function (e: Event) {
    ...
    if (files) {
      const file = files[0];
      try {
        const idFiltered = (await parseCsvToArray(file)).filter((v) => v !== "");
